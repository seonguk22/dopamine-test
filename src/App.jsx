...
import { createPortal } from 'react-dom'; // ✅ Portal 도입
import { 
...
export default function DopamineTest() {
  ...
  const [mounted, setMounted] = useState(false); // ✅ Portal용 마운트 상태
  const resultRef = useRef(null);
  const shareCardRef = useRef(null);

  useEffect(() => { setMounted(true); }, []); // ✅ 클라이언트 사이드 렌더링 보장
  ...
  const shareToKakao = async () => {
    ...
    if (state.step === 'result') {
      try {
        ...
        // ✅ [개선] 폰트 대기 및 2프레임 지연으로 렌더링 확실성 보장
        if (document.fonts?.ready) await document.fonts.ready;
        await new Promise(r => requestAnimationFrame(() => r(null)));
        await new Promise(r => requestAnimationFrame(() => r(null)));

        // ✅ [개선] 캡처 시 사이즈를 명시하여 레이아웃 흔들림 차단
        ...
        if (w) {
          w.document.open();
          w.document.write(`<html><body style="margin:0;background:#000;display:flex;justify-content:center;align-items:center;"><img src="${dataUrl}" style="max-width:100%;height:auto;"/></body></html>`);
          w.document.close();
        }
  ...
  // ✅ [필수] 캡처용 카드를 Portal로 정의 (body 직속 렌더링)
  const CaptureCard = (
    <div
      ref={shareCardRef}
      style={{
        ...
        left: 0,
        top: 0,
        transform: 'translateX(-200vw)', // ✅ overflow-hidden 영향을 받지 않도록 뷰포트 단위 이동
        width: '500px',
        height: '500px',
        backgroundColor: '#0a0a0a',
        pointerEvents: 'none',
    ...
    >
      ...
      <div style={{ fontSize: '48px', fontWeight: '900', marginBottom: '20px', textAlign: 'center', lineHeight: '1.1' }}>
        {trans.title}
      </div>
      ...
      <div style={{ position: 'absolute', bottom: '35px', left: 0, right: 0, textAlign: 'center', fontSize: '14px', opacity: '0.5' }}>
  ...
  return (
    <>
      ...
      {/* ✅ [해결] 캡처용 카드를 Portal을 통해 body 직속으로 렌더링 (부모 overflow 차단) */}
